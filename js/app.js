...
function examples() {
  $('div.example').each(function() {
    ...
    $form.submit(function(event) {
	  ...
	  var inputCommand = ($input.val().replaceAll(" ","/"));
	  
	  var commandindex = inputCommand.indexOf("/");
	  var curcommand = inputCommand;
	  
	  if(commandindex>=0){
		  curcommand = inputCommand.substring(0,commandindex);
	  }
      ...
      $.ajax({
        type: "get",
        url: "http://118.26.18.69:7379/"+inputCommand,
        //data: $form.serialize(),
        complete: function(xhr, textStatus) {
		  ...
		  if(typeof(obj[curcommand]) === "object"){
			  var pre = $("<pre></pre>").text(obj[curcommand][1]);
		  }else{
		  		var pre = $("<pre></pre>").text(obj[curcommand]);
		  }
